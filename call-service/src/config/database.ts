...
const connectToDatabase = async (): Promise<mongoose.Connection> => {
  ...
  try {
    defaultConnection = await mongoose.createConnection(MONGODB_URI, defaultOptions);
...
const getDbConnection = (dbName?: string | null): mongoose.Connection => {
  if (!defaultConnection) {
    throw new Error('Default database connection not initialized. Call connectToDatabase() first.');
  }
