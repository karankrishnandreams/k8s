...
export const verifyInternalToken = (req: Request, res: Response, next: NextFunction): void => {
  ...
  if (!authHeader || !authHeader.startsWith('Bearer ')) {
    res.status(401).json({
      error: `Missing or invalid Authorization header from ${process.env.EMAILSERVICE_NAME}`,
    });
    return;
  }
  ...
  try {
    ...
    if (typeof decoded === 'object' && decoded.service !== 'api-gateway') {
      res
        .status(403)
        .json({ error: `Forbidden: Invalid service from ${process.env.EMAILSERVICE_NAME}` });
      return;
    }
    ...
    res
      .status(401)
      .json({ error: `Invalid or expired token from ${process.env.EMAILSERVICE_NAME}` });
    return;
  }
};
