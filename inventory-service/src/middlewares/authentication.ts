...
export const authenticateWithSubdomainCheck = (
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      const companyDomain: any = await companyDomainUrlList(dbName);
      ...
      if (!matchedAccountURL) {
        throw createHttpError(
          403,
          "Access restricted to valid workspace subdomains"
        );
      }
      ...
      const workspace = matchedAccountURL.workspace as string | undefined;
      ...
      if (isJWT) {
        ...
        req.user.companyId = matchedAccountURL._id;
...
export const authenticateWithScanToken = (
  isJWT: boolean = true,
  roleKEY: string = ""
) => {
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!decoded.accountURL) {
        throw createHttpError(400, "Token does not contain accountURL");
      }

      // Now fetch company domain list (from main DB or config)
      const companyDomain: any = await companyDomainUrlList(dbName);

      // Find matching accountURL
      const matchedAccountURL = companyDomain.find(
        (com: { account_url: string }) =>
          com.account_url === decoded.accountURL
      );

      if (!matchedAccountURL) {
        throw createHttpError(403, "Invalid workspace or account URL");
      }

      // Set fields for downstream use
      //@ts-ignore
      req.companyObjId = matchedAccountURL._id;
      //@ts-ignore
      req.company_id = matchedAccountURL.company_id;
      //@ts-ignore
      req.account_url = matchedAccountURL.account_url;
      req.headers["x-db-name"] = `${matchedAccountURL.company_id}${process.env.DB_SUFFIX}`;
      req.user.companyId = matchedAccountURL._id;

      // Optional: check role permissions via API call
      /*
      if (roleKEY) {
        const payload = {
        ...
        const rolePermission: CustomAxiosRequestConfig = {
          method: "post",
          url: "/user/public/auth/role-permission",
          data: payload,
        };

        const { data } = await kongAxios(rolePermission);
        if (!data.success) {
          throw createHttpError(403, data.message);
        }
      }
      */
